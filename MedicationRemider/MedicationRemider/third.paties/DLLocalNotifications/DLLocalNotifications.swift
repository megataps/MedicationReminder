...
public class DLNotificationScheduler {
    ...
    public func cancelNotificationWithIdentifier (_ identifier: String) {
        //        let notification = DLQueue.queue.notificationWithIdentifier(identifier)
    ...
    public func printAllNotifications () {
        ...
        UNUserNotificationCenter.current().getPendingNotificationRequests(completionHandler: { (requests) in
            Logger.log("DMMMMM>>>>>>requests.count: \(requests.count)", .e)
            for request  in  requests {
                if let request1 =  request.trigger as?  UNTimeIntervalNotificationTrigger {
                    Logger.log("DMMMMM>>>>>>Timer interval notificaiton: \(request1.nextTriggerDate().debugDescription)")
                }
                if let request2 =  request.trigger as?  UNCalendarNotificationTrigger {
                    if(request2.repeats) {
                        Logger.log("DMMMMM>>>>>>Calendar notification: \(request2.nextTriggerDate().debugDescription) and repeats")
                    } else {
                        Logger.log("DMMMMM>>>>>>Calendar notification: \(request2.nextTriggerDate().debugDescription) does not repeat")
                    }
                }
                if let request3 = request.trigger as? UNLocationNotificationTrigger {
                    
                    Logger.log("DMMMMM>>>>>>Location notification: \(request3.region.debugDescription)")
                }
            }
        })
    }
...
public class DLNotification {
    ...
    public init (identifier: String, alertTitle: String, alertBody: String, date: Date?, repeats: RepeatingInterval ) {
        ...
        Logger.log("DMMMMM>>>>>>self.fireDate: \(self.fireDate)")
